...
def write_to_silver(df, batch_id):
    ...
    if ~df.isEmpty():
        try:
            ...
            dt.merge(
                df.alias("data"), F.expr(f"{index_match} and {ingest_time_match}")
            ).whenMatchedUpdateAll().execute()
            print("Looking for records to insert...")
            dt.merge(
                df.alias("data"), F.expr(index_match)
            ).whenNotMatchedInsertAll().execute()
        except pyspark.sql.utils.AnalysisException:
            print("Table may not exist. Trying to create table...")
            df.write.format("delta").partitionBy("filter_code", "region").saveAsTable(
                silver_table
            )
    else:
...
if process_mode == "streaming":
    ...
    df.writeStream.option("checkpointLocation", checkpoint_path).foreachBatch(
        write_to_silver
